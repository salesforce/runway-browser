...
class ModelsNav {
  ...
  append(locator, packageJSON) {
    let name = packageJSON['runway-label'];
    if (name === undefined) {
      let m = packageJSON.name.match('^runway-model-(.*)$');
      if (m !== null) {
        name = m[1];
      } else {
...
let loadModel = repo => {
  ...
  let trustworthy = new Promise((resolve, reject) => {
    ...
    if (repo.match(new RegExp('^github.com/(SalesforceEng|ongardie)/runway-model-.*', 'i')) !== null) {
    ...
    pageLoaded.then(() => {
      ...
      jQuery('button.continue', modal).click(resolve);
