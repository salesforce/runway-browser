...
let loadModel = repo => {
  ...
  browserRequire.remote(`${repo}/package.json`)
    .catch(err => {
    ...
    .then(packageJSON => {
      ...
      let specPromise = browserRequire.remote(`${repo}/${modelJSON.spec}`);
      let viewPromise = browserRequire.remote(`${repo}/${modelJSON.view}`)
        .catch(err => {
